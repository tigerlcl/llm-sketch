...
def chunk_dirty_slice(args):
    ...
    for i in range(num_slices):
        ...
        all_indices = [(row, col) for row in slice_df.index for col in columns]
        ...
        for _ in range(min(num_missing, len(all_indices))):
            ...
            slice_df.at[row, col] = None  # override
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num-slices', '-s', type=int, default=10, help="number of slices")
    parser.add_argument('--num-rows', '-r', type=int, default=7, help="number of rows per slice")
