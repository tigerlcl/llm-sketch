...
def sketch_prompt() -> ChatPromptTemplate:
    prompt = ChatPromptTemplate.from_messages(
        [
            ...
            [Tuple Sampling]: Select several no-issue records based on above columns and ignore other columns. Show the result in an array of tuple.
            [Rule Mining]: Find out the rule that fit in sampled results and can derive the missing value. Wrap the rule as a Python function in code block.
